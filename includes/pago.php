...
class pago extends db implements crud {
    ...
    public function listarPagosPendientes(){
        return $this->select("distinct p.*", "pagos p join pago_detalle d on p.id = d.id_pago", Array("estatus"=>"'p'"),null,Array("d.id_inmueble"=>"ASC"));
    }
    ...
    public function registrarPago($data) {
        ...
        try {
            ...
            if ($res['suceed'] && !count($res['data']) > 0 ) {
                ...
                if ($res['data'][0]['estatus']=='a') {
                    $resultado['mensaje'].= "\nEl pago ya fue rechazado. Si considera que es un error nuestro, escríbanos a info@grupoinmobiliariotierradelsol.com";
                }
            }
        } catch (Exception $exc) {
            $resultado['suceed'] = false;
        ...
        if ($estatus == 'A') {
            ...
            if ($r['suceed'] == true) {
                if (count($r['data']) > 0) {
                    ...
                    $mensaje.= "Hemos adjuntado su recibo electrónico (" . $n . ").";
                    if ($n < count($r['data'])) {
                        $mensaje . -"<br>No se pudo adjuntar el recibo correspondiente.";
                    }
                }
            }
        }// </editor-fold>
